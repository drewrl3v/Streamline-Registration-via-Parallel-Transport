...
# compute the Karther mean and the tangent vectors
qmean2, alpha_t_arr2 = karcher_mean(srvf_tract2, 1, 7, 5)
...
# Rotate the tangent vector to 'best-overall' fit
Aproj, _ = find_best_rotation(Aproj, Aproj2)

# This is experimental: You can perform a hard registration by solving a matrix equation to match Fourier coefficients
# WARNING: While results should be strictly better than soft registration, it's not a garuntee that we garuntee
#          important shape features we 'warped' with respect to the topology
...
for i, gp in enumerate(recon):
    ...
    for t in range(di): # should be 123
        recon_alpha_t_arr += gp[t] * G[t]

    q_recon, _ = geodesic_flow(qmean, recon_alpha_t_arr, stp = 7)
...
srr = StreamlineLinearRegistration(x0="affine")
